...
Примечание: если "виноватое" выражение находится на одной строчке, то выведено будет только выражение. Если же выражение занимает несколько строчек, то первая строка будет выведена с начала строки, а не выражения. (это просто стилистический выбор) 
...
Можно собрать напрямую без докера простым `mvn package -DskipTests`. Пропустить тесты необходимо, если при клонировании репозитория не был склонирован гит-сабмодуль с тестовыми файлами.

...
2. По описанию `#open-variant-exceptions` предполагается, что нельзя использовать вариант ошибки до того, как он был объявлен. Однако, это условие не проверяется эталонным тайпчекером.

   Данная особенность сохранена в моей реализации тайпчекера
3. Технически по описанию проекта не было явно указано, какая ошибка имеет приоритет - `ERROR_UNEXPECTED_SUBTYPE` или `ERROR_UNEXPECTED_PATTERN_FOR_TYPE`. В моей реализации приоритетнее `ERROR_UNEXPECTED_PATTERN_FOR_TYPE`.
